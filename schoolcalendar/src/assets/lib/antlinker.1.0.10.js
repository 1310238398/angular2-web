 ...
 * fail：func（失败调用）
 */
antlinker.playVideoOnFullView = function (request) {
    antlinker.playVideoOnFullView.success = request.success;
    antlinker.playVideoOnFullView.fail = request.fail;
    if (antlinker.isAndroid) {
        try {
            androidAntLinker.playVideoOnFullView(JSON.stringify(request));
        } catch (err) {
            antlinker.playVideoOnFullView.fail(err);
            console.log("Android playVideoOnFullView not exist");
        }
    } else {
        console.log("unknown device");
    }
 ...
 * fail：func（失败调用）
 */
antlinker.browseOriginalImages = function (request) {
    antlinker.browseOriginalImages.success = request.success;
    antlinker.browseOriginalImages.fail = request.fail;
    if (antlinker.isiOS) {
        try {
            webkit.messageHandlers.browseOriginalImages.postMessage(JSON.stringify(request));
        } catch (err) {
            antlinker.browseOriginalImages.fail(err);
            console.log("iOS browseOriginalImages not exist");
        }
    } else if (antlinker.isAndroid) {
        try {
            androidAntLinker.browseOriginalImages(JSON.stringify(request));
        } catch (err) {
            antlinker.browseOriginalImages.fail(err);
            console.log("Android browseOriginalImages not exist");
        }
    } else {
        console.log("unknown device");
    }
