...
export class HttpService {
  private baseUrl = '/api/appsrv/interface';

  private AccessToken = 'ZQXGWB9MOYSMMTANBV1GIG';
  ...
  POST(params, url = this.baseUrl): Observable < any > {
    const {Router, Method= 'POST',  Body= {} } = params;
    const httpOptions = {
   ...
   const payload = JSON.stringify(Body);
    ...
    return this.httpClient.post < any > (url, {Router: Router, Method: Method, Body: payload}, httpOptions).pipe(catchError(this.handleError));
  }
    format(time, format) {
        var date = {
            "M+": time.getMonth() + 1,
