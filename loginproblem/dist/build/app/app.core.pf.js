...
(function(resourcesUrl){/** @stencil/router global **/
...
(function(resourcesUrl){/** @ionic/core global **/
function configFromURL() {
    var config = {};
    var win = window;
    win.location.search.slice(1)
        .split('&')
        .filter(function (entryText) { return entryText.startsWith('ionic:'); })
        .map(function (entryText) { return entryText.split('='); })
...
Object.defineProperty(Ionic, 'queue', {
    get: function () { return Context['queue']; }
});
// create the Ionic.config from raw config object (if it exists)
// and convert Ionic.config into a ConfigApi that has a get() fn
var config = Ionic['config'] = Context['config'] = new Config(Object.assign({}, Ionic['config'], configFromURL()));
// first see if the mode was set as an attribute on <html>
// which could have been set by the user, or by prerendering
// otherwise get the mode via config settings, and fallback to md
var documentElement = document.documentElement;
var mode = config.get('mode', documentElement.getAttribute('mode') || (isIOS(window) ? 'ios' : 'md'));
...
(function(window, document, Context, namespace) {
  ...
  function attachStyles(plt, domApi, cmpMeta, modeName, elm, customStyle, styleElm) {
    ...
    if (styleTemplate) {
      ...
      if (!appliedStyles[styleModeId]) {
        ...
        customStyle && !customStyle.supportsCssVars && customStyle.addStyle(styleElm);
  ...
  function createPlatformMainLegacy(namespace, Context, win, doc, resourcesUrl, hydratedCssClass, customStyle) {
    ...
    /**
      ...
      // each key in moduleImports is a PascalCased tag name
            name && !name.endsWith('.js') && Object.keys(bundleExports).forEach(function(pascalCasedTagName) {
    ...
    customStyle && customStyle.init(function() {
    ...
    // This is executed by the component's connected callback.
      ...
      if (getLoadedBundle(bundleId)) {
        ...
        // when to request the bundle depends is we're using the css shim or not
                customStyle && !customStyle.supportsCssVars && requestBundleQueue ? 
        // add this to the loadBundleQueue to run when css is ready
        requestBundleQueue.push(function() {
          return requestComponentBundle(cmpMeta, bundleId);
        }) : 
    ...
    function requestComponentBundle(cmpMeta, bundleId) {
      ...
      var useScoped = 2 /* ScopedCss */ === cmpMeta.encapsulation || 1 /* ShadowDom */ === cmpMeta.encapsulation && !domApi.$supportsShadowDom;
  ...
  /*
    ...
    */
  /* tslint:disable */  var StyleNode = /** @class */ function() {
    function StyleNode() {
      this.rules = null;
      this.start = 0;
      this.end = 0;
  ...
  var CustomStyle = /** @class */ function() {
    ...
    CustomStyle.prototype.init = function(cb) {
      var _this = this;
      this.supportsCssVars ? cb && cb() : this.win.requestAnimationFrame(function() {
        var promises = [];
        var linkElms = _this.doc.querySelectorAll('link[rel="stylesheet"][href]');
        for (var i = 0; i < linkElms.length; i++) {
          promises.push(loadLinkStyles(_this.doc, _this, linkElms[i]));
        }
        var styleElms = _this.doc.querySelectorAll('style');
        for (i = 0; i < styleElms.length; i++) {
          promises.push(_this.addStyle(styleElms[i]));
        }
