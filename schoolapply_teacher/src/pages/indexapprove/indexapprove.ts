...
export class IndexApprovePage {
  ...
  NavigationTo(obj) {
    ...
    } else if (obj.flow_code == 'Process_StudentNeedSupport') {
      console.log('贫困生跳转')
      sessionStorage.setItem('dataPass', JSON.stringify(obj));                    //当前任务详细参数
      sessionStorage.setItem('appStatus', JSON.stringify(this.appStatus));           //当前任务ID
      this.navCtrl.push('PoolStudentsDetailPage', { dataPass: obj, appStatus: this.appStatus });

    } else if (obj.flow_code == 'Process_Student_Status_Adjust') {
      this.navCtrl.push('ChangeStatusDetailPage', { dataPass: obj, appStatus: this.appStatus });
    } else if (obj.flow_code == 'Process_StudentChangeClass') {
      this.navCtrl.push('ChangeClassDetailPage', { dataPass: obj, appStatus: this.appStatus });
    } else if (obj.flow_code == 'Process_QingongStudySupport') {
      this.navCtrl.push('QingongStudyDetailPage', { dataPass: obj, appStatus: this.appStatus });
    } else if (obj.flow_code == 'Process_school_activity_apply') {
      this.navCtrl.push('SheTuanDetailPage', { dataPass: obj, appStatus: this.appStatus });
    } else if (obj.flow_code == 'Process_SecondClassRoom') {
      this.navCtrl.push('SecondClassRoomDetailPage', { dataPass: obj, appStatus: this.appStatus });
    }
  }
