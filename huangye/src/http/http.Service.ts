...
export class HttpService {
   ...
   * @param queryParams
   */
  async postJSON1<T>(params: any, url: string = this.baseUrl) {
   /* let obz = new Observable(obs => {
      antlinker.exeUserBasicInfoSdk({}, function (obj, success) {
        obs.next(success.AppWebKey)
      });
    }) ;*/
    try {
      await antlinker.exeUserBasicInfoSdk({}, function (obj, success) {});
      this.AccessTokenL = window["__AppWebkey"];
      let headers = new Headers({'Content-Type': 'application/json', 'AccessToken': this.AccessTokenL});
      let options = new RequestOptions({headers: headers});
      return this.http.post(url, JSON.stringify({
      ...
      console.log('异步Error: ', err.message);
    }
