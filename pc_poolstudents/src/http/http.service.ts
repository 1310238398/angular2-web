...
export class HttpService {
  ...
  private apiFlowUrl: string = ServelUrl.Url.apiFlowUrl;
  private thirdUrl: string = ServelUrl.Url.apiUrl1;

  private AccessTokenL: string = 'NDX5NQIMNJ2TV3PGERFTXA';
   ...
   * @param {*} [params] 请求参数
   */
  POSTNotLaoding<T>(params: any, url: string = this.baseUrl): Observable<any> {
    let headers = new Headers({
      'Content-Type': 'application/json',
      'AccessToken': window["__AppWebkey"] || this.AccessTokenL
    });
 ...
 * @param {*} [params] 请求参数
 */
  postFLOW<T>(params: any, url: string = this.apiFlowUrl): Observable<any> {
    let headers = new Headers({
      'Content-Type': 'application/json',
      'AccessToken': window["__AppWebkey"] || this.AccessTokenL
    });
