...
@IonicPage()
@Component({
  selector: 'nameliststudent',
  templateUrl: 'nameliststudent.html',
})
export class NameListStudentPage {
  ...
  mengGo = true;  //加载动画
  pagePass = '';  //从哪个页面进入的
  UserRoleList = '' //当前用户的IntelUserCode
  ...
  loadNameFirst() {
    this.http.postJSON({
      Router: ServelUrl.Url.studentstulist,
      Method: 'POST',
      Body: {
        ...
        TeacherIntelUserCode: this.dataPassArr.TeacherIntelUserCode,
        IntelUserCode: this.dataPassArr.IntelUserCode,  //IntelUserCode|
  ...
  loadNameOver() {
    this.http.postJSON({
      Router: ServelUrl.Url.studentstulisthis,
      Method: 'POST',
      Body: {
  ...
  submitInfo() {
    ...
    this.http.postJSON({
      Router: ServelUrl.Url.studentsavestulist,
      Method: 'POST',
      Body: {
        recordID: this.dataPassArr.RecordID,
        attendance: strSpeak,
      ...
      data => {
        if (!data.FeedbackCode && this.pagePass == 'index') {
          ...
          var that = this;
          setTimeout(function () {
            that.navCtrl.push('IndexStudentPage');
          }, '1500');
        } else if (!data.FeedbackCode && this.pagePass == 'history') {
          this.HelpUtils.toastPopTop('修改完成');
          var that = this;
          setTimeout(function () {
            that.navCtrl.push('HistoryRecordStudentPage', { exitData: that.exitData });
          }, '1500');
  ...
  loadUserRoleList() {
    this.http.postJSON({
      ...
      data => {
        if (!data.FeedbackCode && data.Data != null) {
          this.UserRoleList = data.Data.IntelUserCode;
          if (this.UserRoleList != this.dataPassArr.IntelUserCode && this.dataPassArr.Remark1 != '') {
            this.markDn = false
            this.dataPassArr.readName = '2'
          }
        }
      },
      err => console.log(err));
