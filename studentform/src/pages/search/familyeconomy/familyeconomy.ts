...
//添加家庭成员
            ...
            buttons: [
              {
                text: '确定',
                handler: () => {
                    this.Learning.pop({StartDate: "", EndDate: "", Details: "",});
                }
              },
              {
                text: '取消',
    ...
    addsoPage() {
        ...
        if (this.RewardsAndPunishments.length + 1 <= 5) {
            this.RewardsAndPunishments.push({StartDate: "", EndDate: "", Details: "",});
        } else {
            this.HelpUtils.toastPop("最多只能添加5个！");
        }
    }
            ...
            buttons: [
              {
                text: '确定',
                handler: () => {
                    this.RewardsAndPunishments.pop({StartDate: "", EndDate: "", Details: "",});
                }
              },
              {
                text: '取消',
    ...
    onDetailPageTJ() {
        ...
        for (w = 0; w < this.Learning.length; w++) {
            if (!this.Learning[w].StartDate) {
                ...
                this.HelpUtils.toastPop('请填写起始日期！');
                return;

            }
            if (!this.Learning[w].EndDate.trim()) {
                this.HelpUtils.toastPop('请填写结束日期！');
                return;
        ...
        for (t = 0; t < this.RewardsAndPunishments.length; t++) {
            if (!this.RewardsAndPunishments[t].StartDate.trim()) {
                ...
                this.HelpUtils.toastPop('请填写开始时间！');
                return;

            }
            if (!this.RewardsAndPunishments[t].EndDate.trim()) {
                this.HelpUtils.toastPop('请填写结束时间！');
                return;
