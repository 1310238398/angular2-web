...
export class StudentApplyDetailPage {
  ...
  constructor(private helpUtil:HelpUtils,private DomS: DomSanitizer, private http: HttpService, private commonService: CommonService, private navCtrl: NavController, private params: NavParams, private HelpUtils: HelpUtils) {
    ...
    if (this.leaveApply.ApproveStatus == 4) {
      this.commonService.getStudentApplyLocationAndAuthReason({TaskCode: this.leaveApply.TaskCode}).then(data => {
   ...
   */
  onUnApprove(apply) {
    this.navCtrl.push('UnapprovedPage', {apply: apply, TaskCode: this.leaveApply.TaskCode});
  }
  ...
  onApprove() {
    this.http.postJSON({
      Router: ServelUrl.Url.StaffApprovalHandleLeaveApplication,
      Method: 'POST',
      Body: {
  ...
  onApprpveLeave() {
    this.http.postJSON({
      Router: ServelUrl.Url.StaffApprovalHandleFakeLeaveApplication,
      Method: 'POST',
      Body: {
        TaskCode: this.leaveApply.TaskCode
      }
