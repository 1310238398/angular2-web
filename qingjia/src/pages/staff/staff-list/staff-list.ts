...
*/
  ...
  doRefresh(refresher, ApproveStatus) {
    this.Page = 0;
    this.moreData = true;
    this.http.postJSON({
      Router: ServelUrl.Url.querystudentleaveapplication,
   ...
   * 下拉加载
   * */
  doInfinite(infiniteScroll, ApproveStatus, key) {
    this.Page++;
    this.http.postJSON({
      Router: ServelUrl.Url.querystudentleaveapplication,
      Method: 'POST',
