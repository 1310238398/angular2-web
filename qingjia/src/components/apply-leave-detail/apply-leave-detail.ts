      ...
      <ion-card-content>
        <div>
          ...
          <div class="zj-text-indent">请假期间，本人将去往<span>{{OutPlace.CodeName}}</span>，
            ...
            联系电话:<span class="line" (click)="call()">{{leaveApply.EmergencyNumber}}</span>。
          </div>
...
export class ApplyLeaveDetailComponent {
  ...
  call() {
    if (this.self_leaveApply.EmergencyNumber) {
      antlinker.onTel(
        {
          tel: this.self_leaveApply.EmergencyNumber,
          text: this.self_leaveApply.EmergencyPerson,
          success: function () {
