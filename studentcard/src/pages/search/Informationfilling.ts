...
export class InformationfillingPage {
    ...
    ionViewDidEnter() {
        this.http.postJSON({
            Router: '/web/system/parameter/sysparameterauto',
            Method: 'POST',
            Body: {parameter: ["Campus"]}
        }).then(
            res => {
                if (!res.FeedbackCode) {
                  // localStorage.setItem('logo',res.Data[0].Value);
                  this.schoolName=res.Data[0].Value;
                   localStorage.setItem('schoolName',res.Data[0].Value);
                }
            });
    }
